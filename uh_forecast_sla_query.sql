...
select
  ...
  -- Updated SLA thresholds based on horizon
  , case 
      when a.horizon = 0 then 
        case when error_abs <= (0.3 / 100) then 1 else 0 end
      when a.horizon between 1 and 4 then 
        case when error_abs <= (0.3 / 100) then 1 else 0 end
      when a.horizon between 5 and 13 then 
        case when error_abs <= (0.45 / 100) then 1 else 0 end
      else 0
    end as within_SLA
  , b.total_delivs
...
select 
  ...
  -- Updated SLA for global level based on horizon
  , case 
      when horizon = 0 then 
        case when error_abs_agg <= (0.3 / 100) then 1 else 0 end
      when horizon between 1 and 4 then 
        case when error_abs_agg <= (0.3 / 100) then 1 else 0 end
      when horizon between 5 and 13 then 
        case when error_abs_agg <= (0.45 / 100) then 1 else 0 end
      else 0
    end as within_SLA 
...
, metric_base_long_format as (
  select 'Actuals' as metric_cat, 0 as metric_cat_order, 0 as metric_name_order, 
    forecast_created_week, forecast_week, horizon, scenario, config, submarket_id, 
    within_sla, total_delivs, aggregation_level, large_sm_flag, ideal_online_hours, 
    total_delivs_bucket, total_delivs_bucket_order,
    'Actual_UH' as metric_name, actual_uh as metric_value 
  from combined_with_flags 
  union all
  
  select metric_cat, metric_cat_order, 1 as metric_name_order, 
    forecast_created_week, forecast_week, horizon, scenario, config, submarket_id, 
    within_sla, total_delivs, aggregation_level, large_sm_flag, ideal_online_hours, 
    total_delivs_bucket, total_delivs_bucket_order,
    'Forecast_UH' as metric_name, fcst_uh as metric_value 
  from combined_with_flags 
  union all
  
  select metric_cat, metric_cat_order, 2 as metric_name_order, 
    forecast_created_week, forecast_week, horizon, scenario, config, submarket_id, 
    within_sla, total_delivs, aggregation_level, large_sm_flag, ideal_online_hours, 
    total_delivs_bucket, total_delivs_bucket_order,
    'Error' as metric_name, error as metric_value 
  from combined_with_flags 
  union all
  
  select metric_cat, metric_cat_order, 3 as metric_name_order, 
    forecast_created_week, forecast_week, horizon, scenario, config, submarket_id, 
    within_sla, total_delivs, aggregation_level, large_sm_flag, ideal_online_hours, 
    total_delivs_bucket, total_delivs_bucket_order,
    'Error_ABS' as metric_name, error_abs as metric_value 
  from combined_with_flags 
  union all
  
  select metric_cat, metric_cat_order, 4 as metric_name_order, 
    forecast_created_week, forecast_week, horizon, scenario, config, submarket_id, 
    within_sla, total_delivs, aggregation_level, large_sm_flag, ideal_online_hours, 
    total_delivs_bucket, total_delivs_bucket_order,
    'Error_PCT' as metric_name, error_pct as metric_value 
  from combined_with_flags 
  union all
  
  select metric_cat, metric_cat_order, 5 as metric_name_order, 
    forecast_created_week, forecast_week, horizon, scenario, config, submarket_id, 
    within_sla, total_delivs, aggregation_level, large_sm_flag, ideal_online_hours, 
    total_delivs_bucket, total_delivs_bucket_order,
    'Error_ABS_PCT' as metric_name, error_abs_pct as metric_value 
  from combined_with_flags
)

-- Final output with SLA tracking metrics
, sla_summary as (
  select 
    forecast_created_week,
    scenario,
    ...
    aggregation_level,
    horizon,
    count(distinct forecast_week) as total_weeks,
    sum(within_sla) as weeks_within_sla,
    round(div0(sum(within_sla), count(distinct forecast_week)) * 100, 2) as sla_pct,
    -- Special handling for w0 forecast SLA (needs >11/13 weeks)
    case 
      when horizon = 0 then 
        case when sum(within_sla) > 11 and count(distinct forecast_week) = 13 then 1 else 0 end
      else 
        case when div0(sum(within_sla), count(distinct forecast_week)) = 1 then 1 else 0 end
    end as meets_sla_requirement
  from combined_with_flags
  group by all
)

-- Choose one of the following outputs based on your dashboard needs:

-- Option 1: Detailed metrics view
select * from metric_base_long_format

-- Option 2: SLA summary view (uncomment to use)
-- select * from sla_summary
-- order by forecast_created_week desc, horizon

-- Option 3: Combined view with SLA metrics (uncomment to use)
-- select 
--   m.*,
--   s.total_weeks,
--   s.weeks_within_sla,
--   s.sla_pct,
--   s.meets_sla_requirement
-- from metric_base_long_format m
-- left join sla_summary s
--   on m.forecast_created_week = s.forecast_created_week
--   and m.scenario = s.scenario
--   and m.config = s.config
--   and m.aggregation_level = s.aggregation_level
