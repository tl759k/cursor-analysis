...
select 
  ...
  , sum(dx_cnt) over (partition by active_week, ds.submarket_id) total_dx
  , div0(dx_cnt, total_dx) as dx_share
  , sum(ds.adj_shift_seconds) / 3600 as hours
  , sum(hours) over(partition by active_week, ds.submarket_id) total_hours
...
select
  ...
  , case when month(active_week) between 7 and 8 then 'summer' else 'not_summer' end as season
  , submarket_id
  , submarket_name
  , avg(dx_cnt) dx_cnt
...
group by all
order by year, submarket_id asc