...
--   , b.msa_title
  ...
  , sum(dx_cnt) over (partition by active_month, ds.submarket_id) total_dx
    -- , sum(dx_cnt) over (partition by active_month, b.cbsa_code) total_dx
  , div0(dx_cnt, total_dx) as dx_share
  , sum(ds.adj_shift_seconds) / 3600 as hours
  , sum(hours) over(partition by active_month, ds.submarket_id) total_hours
...
from prep
where 1=1 
  and year(active_month) = 2024
  and age_bucket = '1-age_18_to_24'
group by all
order by active_month, submarket_id asc